...
db.serialize(() => {
  ...
  // Таблиця товарів у кошику
  db.run(`
    CREATE TABLE IF NOT EXISTS cart_items (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      cart_id INTEGER NOT NULL,   -- Зв'язок з кошиком
      product_id INTEGER NOT NULL,  -- Зв'язок з продуктом
      selected_option TEXT,        -- Вибраний варіант
      quantity INTEGER,            -- Кількість товару
      price REAL,                  -- Ціна продукту
      FOREIGN KEY(cart_id) REFERENCES carts(id),
      FOREIGN KEY(product_id) REFERENCES products(id)
    )
  `, (err) => {
    if (err) {
      console.error("Помилка при створенні таблиці cart_items:", err.message);
    } else {
      console.log("Таблиця cart_items створена або вже існує.");
    }
  });
